...
def view_contacts(request):
    ...
    results = requests.get(f"/coreapi/close-contacts?uid={subject}&diagnosis-date={date}").json()
...
def view_venues(request):
    ...
    subject = request.GET.get("uid")
    date = request.GET.get("diagnosis-date")
    results = requests.get(f"/coreapi/visited-venues?uid={subject}&diagnosis-date={date}").json()
    for result in results:
